...
class SimpleTenderModel:
    ...
    def train(self, labelled_tenders):
        ...
        # TODO: simplify
        contract_dataframe = {"title": titles, "label": labels}
        df = pd.DataFrame(contract_dataframe)
        X = df["title"]
        ylabels = df["label"]
        ...
        logger.info(f"accuracy: {self.pipe.score(X_test, y_test)} , tested with {len(X_test)} instances")
